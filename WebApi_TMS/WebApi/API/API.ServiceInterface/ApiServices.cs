...
namespace WebApi.ServiceInterface
{
    ...
    {        
        ...
        public object Any(ServiceModel.TMS.Tobk request)
        {
            ...
            {
                ServiceInterface.TMS.TableService ts = new ServiceInterface.TMS.TableService();
                ts.TS_tjms2(auth, request, tms_tobk_Logic, ecr, this.Request.Headers.GetValues("Signature"), this.Request.RawUrl);
            }
            catch (Exception ex) { cr(ecr, ex); }
            return ecr;
        }

        public ServiceModel.TMS.tjms_logic tms_tjms_Logic { get; set; }
        public object Any(ServiceModel.TMS.tjms request)
        {
            CommonResponse ecr = new CommonResponse();
            ecr.initial();
            try
            {
                ServiceInterface.TMS.TableService ts = new ServiceInterface.TMS.TableService();
                ts.TS_tjms(auth, request, tms_tjms_Logic, ecr, this.Request.Headers.GetValues("Signature"), this.Request.RawUrl);
            }
            catch (Exception ex) { cr(ecr, ex); }
            return ecr;
        }
